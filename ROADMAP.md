...
Living document. Current ship: **v4.3.0** (2026-04-17). See [CHANGELOG.md](CHANGELOG.md) for what's landed.
...
### 1.1 Silent / unattended CLI mode
...
- **Why this is a core improvement:** today the CLI prompts nothing (good) but also has no way to auto-reboot (bad for imaging). Patching at scale is the function; this closes the gap.
- **Where:** `Program.cs`, threads through `PatchService.Install(unattended: true, ...)`.

### 1.2 Tuning profile import/export (JSON)
...
- **Why this is a core improvement:** tuning profiles are already shipped — users currently have no way to share or replicate them across machines.

### 1.3 Benchmark diff CLI (`compare-benchmarks`)
...
- **Why this is a core improvement:** verifies the patch actually delivered the speedup it promised. Enables CI/CD gates on driver changes.

### 1.4 Config schema migration
...
- **Why this is a core improvement:** protects the tuning/verification/recovery-kit state users accumulate across releases.

### 1.5 Recovery-kit CTA on hero card
...
- **Why this is a core improvement:** the recovery kit is the safety net for the patch. Users skipping it is the highest-cost mistake this tool can allow.
